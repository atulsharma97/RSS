...
const columns = [
  { field: "shaka_id", headerName: "आई डी", width: 130 },
  { field: "shaka_name", headerName: "शाखा", width: 150 },
...
const ShakaScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const { data } = await axios.get(`${apibaseUrl}/get/shaka`);
        if (data.status === 200) {
          SetUser(data.data);
          setLoading(false);
    ...
    <>
      {loading ? (
        ...
        <>
          ...
          <div className="data-table-shakha">
            <DataGrid
